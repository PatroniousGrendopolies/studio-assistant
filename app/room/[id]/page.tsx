...
export default function ChatPage() {
  ...
  const [showStaffHelp, setShowStaffHelp] = useState(false);
  ...
  return (
    <div className="flex h-dvh flex-col bg-white text-black">
      ...
      {/* Staff Help Modal */}
      {showStaffHelp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowStaffHelp(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-black">Need help from staff?</h2>
            <p className="mt-2 text-sm text-gray-600">
              If the AI assistant can&apos;t resolve your issue, reach out to the studio team directly.
            </p>
            <div className="mt-4 space-y-3">
              <a
                href="tel:+15146798761"
                className="flex items-center gap-3 rounded-xl border border-gray-200 px-4 py-3 text-sm text-black transition-colors hover:bg-gray-50"
              >
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500">(514) 679-8761</div>
                </div>
              </a>
            </div>
            <button
              type="button"
              onClick={() => setShowStaffHelp(false)}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto bg-gray-50 px-4 py-4">
        {messages.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center gap-4 px-4">
            ...
            <div className="grid w-full max-w-sm grid-cols-2 gap-2">
              {SYMPTOM_CATEGORIES.slice(0, 4).map((cat) => (
                <button
                  key={cat.id}
                  type="button"
                  onClick={() => handleSymptomTap(cat.message)}
                  className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-left text-xs text-[#303133] transition-colors hover:border-gray-400 hover:bg-gray-50"
                >
        ...
        <div className="mx-auto flex max-w-2xl flex-col gap-3">
          ...
          {error && (
            <div className="flex justify-start">
              <div className="max-w-[85%] rounded-2xl border border-red-200 bg-red-50 px-4 py-2.5 text-sm text-red-800">
